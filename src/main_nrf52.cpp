...
int main(void) {
  ...
  // loop
  while (1) {
    // 1. On régénère le message d'Auth (Timestamp + Signature) à chaque itération
    build_auth_link_message(&UAS_data);

    // 2. On met à jour les données pour l'émetteur Bluetooth
    squitter.foreground(&UAS_data);

    // cpu sleep - On réduit à 200ms pour être très réactif
    k_sleep(K_MSEC(200));
...
strncpy(UAS_data.OperatorID.OperatorId, "200100303ff8040273c6581cccde3998", ODID_ID_SIZE);
